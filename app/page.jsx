...
function OfframpTab({isConnected,openConnectModal,setTab}){
...
export default function RefiFi(){
  ...
  const runTx=async()=>{
    ...
    const defs=[
      ...
      {label:`Off-ramp ${fmtU(debt)} USDC → USD`,key:"ramp"},
    ];
    ...
    try{
      ...
      log(3,"done","usdc-ready");setTxBusy(false);setTxDone(true);await sleep(900);setTab("dashboard");
    }catch(err){const fi=txRows.findIndex((_,i)=>txRows[i]?.status==="loading");if(fi>=0)log(fi,"error");setTxBusy(false);}
  };
  ...
  return(
    <div style={{minHeight:"100vh",background:"#04060f",color:"#dde0f0",fontFamily:"'Outfit',sans-serif",overflowX:"hidden"}}>
      ...
      {/* NAV */}
      <nav style={{padding:"14px 26px",display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:"1px solid rgba(255,255,255,.04)",position:"sticky",top:0,zIndex:50,background:"rgba(4,6,15,.92)",backdropFilter:"blur(18px)"}}>
      ...
      {tab==="offramp"&&(
        <div style={{maxWidth:780,margin:"0 auto",padding:"36px 20px 60px"}} className="fu">
          <OfframpTab isConnected={isConnected} openConnectModal={openConnectModal} setTab={setTab}/>
        </div>
      )}
        ...
        <div style={{maxWidth:980,margin:"0 auto",padding:"36px 20px 60px"}} className="fu">
          {!txDone?(
            <div style={{textAlign:"center",padding:"70px 20px"}}>
              ...
              <p style={{fontSize:14,color:"#1e2040",marginBottom:22}}>Complete a refinance to see your live position dashboard</p>
            ...
            <div className="card fu" key={step} style={{padding:28}}>
              ...
              {step===3&&(<>
                ...
                <div style={{background:"rgba(255,255,255,.02)",borderRadius:12,padding:16,marginBottom:16}}>
                  {[["Refinancing",`${fmtU(debt)} ${DEBTS.find(d=>d.id===dtype)?.name}`],[`Collateral`,`${qty} ${asset.sym} (~${fmtU(colVal)})`],["Protocol",PROTOCOLS.find(p=>p.id===proto)?.name],["Borrow APR",`${dRate}%`],["Health Factor",hf.toFixed(2)],["Annual Savings",fmtU(Math.round(savings))]].map(([k,v],i,arr)=>(
